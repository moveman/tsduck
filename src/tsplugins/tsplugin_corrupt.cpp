...
namespace ts {
    ...
    private:
        ...
        std::vector<Executor *> _allExecutors = std::vector<Executor *> {&_ccErrorExecutor, &_scrambleExecutor, &_pidMapExecutor, &_ptsOffsetExecutor, &_bitErrorExecutor};
...
bool ScriptPlayer::setScript(ts::UString script)
{
	...
	// TODO support map of count vs command
    ...
    for (const auto &action : actions) {
    	...
    	if (time_command.size() != 2) {
    		_corrupter->error(u"script's action is not in expected format: %s", {action});
    		return false;
    	}
    	ts::UStringVector unit_time;
    	time_command[0].split(unit_time, u' ', true, true);
    	if (unit_time.size() != 2) {
    		_corrupter->error(u"script action's time is not in expected format: %s", {time_command[0]});
    		return false;
    	}
    	uint64_t timeMs;
...
{
	_corrupter->debug(u"handlePMT pid %d", {pid});
	if (_pmts.count(pid)) {
